...
def train_single(number_of_agents = 2, number_of_episodes = 1000, 
    ...
    state, reward = INS_Env.env.reset()  # from original KMC code --- initial reset
    
    print('Successfully reset the Environment ')

    ''' Shape, size of the state '''
    state_shape = INS_Env.env.state.shape
    ...
    dim_actions = 2  # integer 0,1,2 for a vector of length 3, 27 combinations in total
    ...
    for iter in range(100):
        ...
        state, reward = INS_Env.env.reset()
        ...
        while not done:
            ...
            new_observation, reward, done = INS_Env.env.step(action)
