...
from .vpg import CL_KmcEnv, CL_SimplePolicyGradient, CL_NeuralNet
...
def train_single(wdir=None, input_path=None, number_of_agents = 2, number_of_episodes = 1000, 
            number_of_timesteps = 10, num_hidden_nodes_1 = 500, num_hidden_nodes_2 = 200,
            actions_space = [0, 1, 2]):
    actions_combinations = np.array([a for a in itertools.product(actions_space, repeat=3)])

    ''' Generate Single KMC Env instance '''
    INS_KmcEnv = CL_KmcEnv(agent_idx=0, box=[4, 8, 4], box_extension=64, target_roughness=0.98, episodes=number_of_episodes,
                            wdir=wdir, reward_type = 'gaussian', reward_multiplier=1000, reward_tolerance=1.0,
                            rates_spread=0.5, rates_adjustment=0.05)
    # env = INS_KmcEnv.env
    print('--- Successfully intialized the KmcEnv')

    
    print('Successfully reset the KmcEnv ')

    state, reward = INS_KmcEnv.env.reset()  # from original KMC code --- initial reset
    print('Successfully reset the KmcEnv ')

    ''' Shape, size of the state '''
    state_shape = INS_KmcEnv.env.state.shape
    state_dim = state_shape[0] * state_shape[1]  # Input dimension for flattened 2D image
    print('--- The input dimension is {}'.format(state_dim))
    dim_actions = 27  # integer 0,1,2 for a vector of length 3, 27 combinations in total
    learning_episode_arr = np.zeros((INS_KmcEnv.episodes, 1))
    ...
    for iter in range(100):
        ...
        while not done:
            ...
            ''' TF 2.0 native code '''
            prob_weights, _ = NeuralNet(observation, tf_actions=None,  bool_actions_prob=True, bool_negative_log_prob=False)
            # print('printing prob_weights type')
            #
            # print(type(prob_weights.numpy()))
            # print(prob_weights.numpy())
            ...
            new_observation, reward, done = INS_KmcEnv.env.step(action)
            ...
            observation = np.reshape(new_observation, (1, state_dim))
